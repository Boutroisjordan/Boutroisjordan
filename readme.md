# Hi, I'm Jordan 👋

> Fullstack Developer & DevOps · Normandy, France

- 🏢 Currently working at **[Groupe Magnolia](https://www.groupe-magnolia.fr)** as a Fullstack & DevOps developer
- 🏠 Tinkering on my homelab — K3S, GitOps, full IaC
- 🌍 Looking for a permanent role from **October 2026** — open to Paris, London, Italy or remote
- 🌐 Check out my portfolio at **[jordanboutrois.fr](https://jordanboutrois.fr)**
- 💬 Ask me anything on my **[LinkedIn](https://linkedin.com/in/jordan-boutrois)**

---

## 🛠 Favorite Tech

<table>
  <tr>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" width="48" height="48" alt="Go" />
      <br>Go
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" width="48" height="48" alt="Kubernetes" />
      <br>Kubernetes
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="48" height="48" alt="Docker" />
      <br>Docker
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/terraform/terraform-original.svg" width="48" height="48" alt="Terraform" />
      <br>Terraform
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/ansible/ansible-original.svg" width="48" height="48" alt="Ansible" />
      <br>Ansible
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/argocd/argocd-original.svg" width="48" height="48" alt="ArgoCD" />
      <br>ArgoCD
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/githubactions/githubactions-original.svg" width="48" height="48" alt="GitHub Actions" />
      <br>GH Actions
    </td>
    <td align="center" width="96">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="48" height="48" alt="Next.js" />
      <br>Next.js
    </td>
  </tr>
</table>

---

## ✅ Milestones

- 🎓 First dev job as an apprentice
- 🚀 First project shipped to production
- 💬 Replied to an open source GitHub issue
- 🏗️ Built a K3S homelab from scratch, fully managed with IaC

## 🎯 What's next

- [ ] Get my Kubernetes certification (CKA)
- [ ] Get my first open source MR merged
- [ ] Land a job in an international company
- [ ] Work daily in English with international teammates
- [ ] Master Go

---

## 📬 Let's connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-jordan--boutrois-0077B5?style=flat-square&logo=linkedin)](https://linkedin.com/in/jordan-boutrois)
[![Portfolio](https://img.shields.io/badge/Portfolio-jordanboutrois.fr-000000?style=flat-square&logo=safari&logoColor=white)](https://jordanboutrois.fr)
[![Email](https://img.shields.io/badge/Email-jordanboutroispro@gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:jordanboutroispro@gmail.com)
